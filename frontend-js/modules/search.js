...
export default class Search {
  //Accessing the DOM by this file to add behavour
  constructor() {
    this.injectHTML();
  ...
  //The methods or tasks to be execute when requested by action or event monitoring
  keyPressHandler() {
    ...
    if (value != "" && value != this.previousValue) {
      ...
      this.showLoaderIcon();
      this.hideResultsArea();
      this.typingWaitTimer = setTimeout(() => this.sendRequest(), 600);
    }
  ...
  sendRequest() {
    axios
      ...
      .catch(() => {
        alert("It failed to access the katbin database");
      });
  }
  ...
  renderResultsHTML(posts) {
    if (posts.length) {
      ...
      this.resultsArea.innerHTML = `<p class="alert alert-danger text-center shadow-sm">Sorry, we could not find any result from your request</p>`;
    }
    this.hideLoaderIcon();
